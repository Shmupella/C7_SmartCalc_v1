...
int number_validation(char *str, int i) {
    int error = 0;
    if (!check_on_operator(str[i+1]) && str[i+1] != '\0' && !check_on_mod(str, i+1) && str[i+1] != '.' && !check_on_number(str[i+1]) && str[i+1] != ')') {
        error = 1;
    }
    if (i != 0 && !check_on_operator(str[i-1]) && !check_on_mod(str, i-1) && str[i-1] != '.' && !check_on_number(str[i-1]) && str[i-1] != '(') {
        error = 1;
    }
    return error;
}
...
int x_validation(char *str, int i) {
    int error = 0;
    if (!check_on_operator(str[i+1]) && str[i+1] != '\0' && !check_on_mod(str, i+1) && str[i+1] != ')') {
        error = 1;
    }
    if (i != 0 && !check_on_operator(str[i-1]) && !check_on_mod(str, i-1) && !check_on_number(str[i-1]) && str[i-1] != '(') {
        error = 1;
    }
    return error;
}
...
int operator_validation(char *str, int i) {
    ...
    if (str[i+1] != '(' && !check_on_number(str[i+1]) && str[i+1] != 'x' && !valid_function(str, &i_for_check_func, &error)) {
        error = 1;          
    }
    if (str[i] == '+' || str[i] == '-') {
        if (str[i-1] != '(' && str[i-1] != ')' && !check_on_number(str[i-1]) && str[i-1] != 'x') {
            error = 1;
        }
    } else {
        if (str[i-1] != ')' && !check_on_number(str[i-1]) && str[i-1] != 'x') {
...
int valid_function(char *str, int *i, int *error) {
    ...
    switch(str[*i]) {
        ...
        case 'l':
            ...
            break;
        case 'a':
            if (str[*i+1] == 'c' && str[*i+2] == 'o' && str[*i+3] == 's' && str[*i+4] == '(') {
                flag = 1;
                *error = microvalid_function(str, *i);
                *i += 4;
            }
            if (str[*i+1] == 's' && str[*i+2] == 'i' && str[*i+3] == 'n' && str[*i+4] == '(') {
                flag = 1;
                *error = microvalid_function(str, *i);
                *i += 4;
            }
            if (str[*i+1] == 't' && str[*i+2] == 'a' && str[*i+3] == 'n' && str[*i+4] == '(') {
                flag = 1;
                *error = microvalid_function(str, *i);
                *i += 4;
            }
